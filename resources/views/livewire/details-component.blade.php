<section class="min-h-screen text-gray-600 body-font overflow-hidden">
    <div class="container px-5 py-12 mx-auto">
        <div class="lg:w-4/5 mx-auto flex flex-wrap">
            <img src="{{asset('/storage/images/products/'.$product->image.'.png')}}"
                 alt="{{ $product->name }}"
                 class="w-1/2 h-full object-cover object-center rounded"
            >
            <div class="lg:w-1/2 w-full lg:pl-10 lg:py-6 mt-6 lg:mt-0">
                <h2 class="text-sm title-font text-gray-500 tracking-widest">{{ $product->category->name }}</h2>
                <h1 class="text-gray-900 text-3xl title-font font-medium mb-1">{{ $product->name }}</h1>

                <p class="leading-relaxed break-words">{{ $product->short_description }}</p>
                <br>
    ...
    <div class="container px-5 py-24 mx-auto">
        <div class="flex-col flex-wrap w-full mb-16">
            <p class="lg:w-1/2 w-full mb-16 leading-relaxed text-gray-500 break-words">{{ $product->description }}</p>
        ...
        <div class="flex flex-wrap -m-4">
            @if(count($relatedProducts) > 0)
                    ...
                    <div class="xl:w-1/4 md:w-1/2 p-4">
                        <div class="bg-gray-100 p-6 rounded-lg">
                            <a href="{{ route('product.details', ['slug' => $product->slug]) }}">
                                <img class="h-40 w-40 rounded w-full object-cover object-center mb-6"
                ...
                @endforeach
            @else
                <p class="pt-30">No Related Products</p>
            @endif
        </div>
    </div>
</section>
